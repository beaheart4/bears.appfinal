...
def get_passing(session: Session = Depends(get_session)):
    ...
    # Note: Using DISTINCT via SQLAlchemy requires custom approach
    # We'll query all and let SQLModel handle serialization
    rows = session.exec(
        select(
            Passing.player,
            Passing.att,
    ...
    # Convert tuples to dicts matching the expected response format
    result = []
    for row in rows:
        result.append({
            "player": row[0],
